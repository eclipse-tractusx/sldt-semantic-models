...
## Versioning
The semantic modeling in Tractus-X follows Semantic Versioning. Since the version is included in the Aspect Model's URI
(i.e. `urn:bamm:io.catenax.batch:1.0.2#Batch`), applications that work on the model may have to adjust even if the APIs
defined by the semantic model may not themselves change. The same phenomenon may occur with the CX-standards that reference
a semantic model. Therefore, the following best practices shall be followed here:
...
- Bugfix: A bugfix is a change in the model that does not add or remove functionality in the model. It is purely syntactical.
Since it will however affect the APIs, the apps and standards must be adjusted accordingly.
